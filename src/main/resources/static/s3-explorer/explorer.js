...
app.factory('SharedService', function($rootScope) {
    ...
    shared.correctClockSkew = function(bucket) {
        var s3 = new AWS.S3({ endpoint: { hostname: "localhost", port: 9000 } });
...
app.controller('SettingsController', function($scope, SharedService) {
    ...
    */
    ...
    $scope.settings = { auth: 'auth', region: 'default', bucket: 'bucket-01',
        entered_bucket: '', selected_bucket: 'bucket-01', view: 'folder', delimiter: '/', prefix: '' };
    $scope.settings.mfa = { use: 'no', code: '' };
    $scope.settings.cred = { accessKeyId: 'minio', secretAccessKey: 'miniosecret', sessionToken: '' };
    $scope.settings.stscred = null;
...
function object2hrefvirt(bucket, key) {
    ...
    // XXX
    return object2hrefpath(bucket, key);
    // var enckey = key.split('/').map(function(x) { return encodeURIComponent(x); }).join('/');
    // return document.location.protocol + '//' + bucket + '.' + host + '/' + enckey;
}
...
function object2hrefpath(bucket, key) {
    var enckey = key.split('/').map(function(x) { return encodeURIComponent(x); }).join('/');
    // XXX
    var host = 'localhost'; // s3.amazonaws.com
    // return document.location.protocol + "//" + host + "/" + bucket + "/" + enckey;
    return document.location.protocol + "//" + host + "/" + bucket;
}
...
$(document).ready(function(){
    ...
    // XXX
    AWS.config.bucket = 'bucket-01';
    AWS.config.endpoint = { hostname: "localhost", port: 9000 };
    AWS.config.credentials = { accessKeyId: 'minio', secretAccessKey: 'miniosecret' };
